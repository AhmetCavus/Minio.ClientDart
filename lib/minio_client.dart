...
class MinioClient {
    ...
    MinioClient(this._baseUrl, this._authService, this._collectionService, this._channelService) {
    ...
    static MinioClient init(String baseUrl) {
      ...
      return MinioClient(baseUrl, AuthService(AuthenticateApi(apiClient)), CollectionService(CollectionApi(apiClient)), ChannelService(ChannelApi(apiClient)));
    }
    ...
    Future<MinioCollection<E>> requestCollection<E extends ConvertableItem>(ItemCreator<E> creator) async {
      ...
      return MinioCollection<E>(collection, _collectionService, _sockets.first, creator);
    }
    ...
    Future<void> subscribeToChannel(String namespace) async {
      ...
      var searchedSocket = _sockets.firstWhere((element) => element.namespace.toLowerCase() == namespace.toLowerCase(), orElse: () => null);
    ...
    void subscribeForBroadcasts(Function(dynamic data) callback, {String namespace = ""}) {
      final socket = _ensureSocketIsCreated(namespace);
      socket.subscribeOn(SOCKET.EVENT_RECEIVE_BROADCAST, callback);
    }

    void subscribeOn<T>(String event, Function(T data) callback, {String namespace = ""}) {
      final socket = _ensureSocketIsCreated(namespace);
      socket.subscribeOn(event, callback);
    }

    void unsubsribeFrom<T>(String event, Function(T data) callback, {String namespace = ""}) {
      final socket = _ensureSocketIsCreated(namespace);
      socket.unsubscribeFrom(event, callback);
    }

    void unsubscribeFromAllEvents() {
      _sockets.forEach((element) { element.unsubscribeFromAllEvents(); });
    }
    ...
    void _clearSockets() {
      _sockets.forEach((element) { element.dispose(); });
      _sockets.clear();
    }

    SocketService _ensureSocketIsCreated(String namespace) {
      final socket = _sockets.firstWhere((element) => element.namespace.toLowerCase() == namespace.toLowerCase(), orElse: () => null);
